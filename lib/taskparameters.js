...
class TaskParameters {
    ...
    _getEnvironmentVariables(environmentVariables, secureEnvironmentVariables) {
        if (environmentVariables) {
            const envPairs = utilities_1.parseForPairs(environmentVariables);
        ...
        if (secureEnvironmentVariables) {
            const securePairs = utilities_1.parseForPairs(secureEnvironmentVariables);
            securePairs.forEach((pair) => {
                let obj = {
                    name: pair[0],
                    secureValue: pair[1],
                };
