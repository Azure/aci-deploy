...
export class TaskParameters {
    ...
    private _environmentVariables: Array<
    ...
    private constructor(endpoint: IAuthorizer) {
        ...
        this._resourceGroup = core.getInput("resource-group", {
            required: true,
        });
        this._commandLine = [];
        ...
        let logAnalyticsWorkspaceKey = core.getInput(
        ...
        if (ipAddress != "Public" && "Private") {
            throw Error(
                "The Value of IP Address must be either Public or Private"
            );
        } else {
        ...
        if (osType != "Linux" && "Windows") {
            throw Error(
                "The Value of OS Type must be either Linux or Windows only!"
            );
        } else {
    ...
    private _getVolumes(
        ...
        if (gitRepoVolumeUrl) {
            ...
            if (!gitRepoMountPath) {
                throw Error(
                    "The Mount Path for GitHub Volume is not specified."
                );
            }
            if (gitRepoDir) {
